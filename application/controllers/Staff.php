...
class Staff extends CI_Controller {
    ...
    public function insert()
    {
        ...
        if($this->form_validation->run() !== false)
        {
            ...
            $login=$this->Home_model->insert_login(array('username'=>$email,'password'=>$state,'usertype'=>2));
            if($login>0)
            {
                $data=$this->Staff_model->insert_staff(array('id'=>$login,'staff_name'=>$name,'gender'=>$gender,'email'=>$email,'mobile'=>$mobile,'dob'=>$dob,'doj'=>$doj,'address'=>$address,'city'=>$city,'state'=>$state,'department_id'=>$department,'pic'=>$image,'added_by'=>$added));
            }
